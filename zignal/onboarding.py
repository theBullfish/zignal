#!/usr/bin/env python3
"""
onboarding.py — Signal first-run setup.

Asks the user:
  1. How they're using Signal (work / personal / combo)
...
Seeds the entity_registry with confirmed data so Signal knows your world
from minute one — before a single session is indexed.

Usage:
    python3 -m signal.onboarding
    or: signal init
"""

from pathlib import Path
from zignal.entity_registry import EntityRegistry
from zignal.entity_detector import detect_entities, scan_for_detection
...
def _ask_mode() -> str:
    _header("Welcome to Signal")
    print("""
  Signal is a personal memory system. To work well, it needs to know
...
""")
    print("  How are you using Signal?")
    print()
...
def _ask_projects(mode: str) -> list:
    ...
    _hr()
    print("""
  What are your main projects? (These help Signal distinguish project
...
def _warn_ambiguous(people: list) -> list:
    ...
    """
    from zignal.entity_registry import COMMON_ENGLISH_WORDS
...
def _generate_aaak_bootstrap(
    ...
    """
    signal_dir = Path(config_dir) if config_dir else Path.home() / ".signal"
    ...
    registry_lines = [
        "# AAAK Entity Registry",
        "# Auto-generated by signal init. Update as needed.",
        "",
        "## People",
    ]
    for p in people:
    ...
    (signal_dir / "aaak_entities.md").write_text("\n".join(registry_lines))
    ...
    (signal_dir / "critical_facts.md").write_text("\n".join(facts_lines))
...
def run_onboarding(
  ...
  Signal will check the context before treating them as person names.
...
""")
    ...
    print("\n  AAAK entity registry: ~/.signal/aaak_entities.md")
    print("  Critical facts bootstrap: ~/.signal/critical_facts.md")
